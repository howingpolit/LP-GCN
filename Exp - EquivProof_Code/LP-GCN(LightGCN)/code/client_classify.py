...
def find_minimum_user_set_sparse(interaction_matrix: csr_matrix):
    ...
    #
    # one_cols=set(k for k in range(interaction_matrix.shape[1]) if col_sums[0, k] == 1)
    # print("one_cols:",len(one_cols))
    #
    # one_rows=set(i for i in range(interaction_matrix.shape[0]) if row_sums[i, 0] == 1)
    # print("one_rows:",len(one_rows))
    #
    ...
    print("The union of the found items has been found")
...
if __name__ == '__main__':
    data=Dataloader()
    result=find_minimum_user_set_sparse(data.train_user_item_matrix)
