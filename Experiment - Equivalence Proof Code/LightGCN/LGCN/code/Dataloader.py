...
class Dataloader(Dataset):
    def __init__(self,configues=configues):
        ...
        self.adjacency_matrix=self.adjacency_matrix.coalesce().to(configues["device"])

        #pre_calculate(for sample)
        # self.all_pos_items=self.get_all_pos_items(list(range(self.user_number)))
    ...
    def get_adjacency_matrix(self):
        ...
        adj_matrix=sp.dok_matrix((self.user_number+self.item_number,self.user_number+self.item_number),dtype=np.float32)
        adj_matrix=adj_matrix.tolil()
        R=self.train_user_item_matrix.tolil()
        adj_matrix[:self.user_number,self.user_number:]=R
        adj_matrix[self.user_number:,:self.user_number]=R.T
        ...
        index=np.where(row_sum==0)
        # print(len(index[0]))
        # print(index)
        # print(self.item_number)
        # print(self.hash_i_to_u_train[948])
        # print(self.train_user_item_matrix[])
        # if np.any(row_sum==0):
        #     print("zero")
        d_inv=np.power(row_sum,-0.5).flatten()
        has_inf = np.isinf(d_inv).any()
        if has_inf:
            print("The array contains inf elements.")
        else:
            print("The array does not contain inf elements.")
